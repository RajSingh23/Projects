...
struct
    ...
    let rec nextThing () =
      ...
      with CloseParenToken ->
        raise (Can'tParse "Can't Parse: close paren") |
      EndToken ->
        raise (Can'tParse "Can't Parse: end token") |
      NumberToken n ->
        token := nextToken () ;
        Number n |
      OpenParenToken ->
    ...
    and nextThings () = 
      ...
      with CloseParenToken ->
        ...
        Nil |
      EndToken ->
        raise (Can'tParse "Can't Parse: reached end without close paren") |
      _ ->
        let t = nextThing ()
        in
        Cons (t, nextThings())
   ...
   (Cons (Symbol "lambda",
     Cons (Cons (Symbol "n", Nil),
      Cons
       ...
       Nil))),
   Nil)))
Exception: Parser.Can'tParse "Can't Parse: end token".
